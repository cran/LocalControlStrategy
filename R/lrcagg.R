...
function (x, envir, show="all", breaks="Sturges", ...) 
{
	if (!is.environment(envir)) 
        stop("The envir argument to plot.lrcagg() must be an existing environment ...from LCsetup().")	
...
function (K, envir) 
{
    ...
    if (!is.environment(envir)) 
        stop("The envir argument to lrcagg() must be an existing environment ...from LCsetup().")
    if (K < 2) { 
